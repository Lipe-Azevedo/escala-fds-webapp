...
export default function SwapsPage() {
    ...
    const updateSwapStatus = async (swapId: number, status: 'approved' | 'rejected', involvedId: number | null) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.message || 'Falha ao atualizar o status da troca.');
            }
            fetchPageData();
        } catch (err: any) {
            setError(err.message);
    ...
    if (isLoading && !swaps.length) return <p>A carregar solicitações...</p>;
    if (error) return <p style={{ color: '#f87171' }}>{error}</p>;
    if (!user) return <p>Utilizador não encontrado. Por favor, faça login novamente.</p>;
